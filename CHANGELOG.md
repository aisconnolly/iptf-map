...
- refactor(vendor): restructure the TACEO Merces vendor page as a multi-solution [TACEO](vendors/taceo.md) page following the EY/Paladin precedent. Adds the TACEO Identity solution, fills the OPRF sub-approach `example_vendors` in [approach-private-identity](approaches/approach-private-identity.md), and adds TACEO to [domains/identity-compliance](domains/identity-compliance.md) adjacent vendors. Renames `vendors/taceo-merces.md` to `vendors/taceo.md` and updates 8 cross-references.
